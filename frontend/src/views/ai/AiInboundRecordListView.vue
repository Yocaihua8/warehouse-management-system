  ...
  <div class="page-container">
    <el-card shadow="never" class="page-card">
      ...
      >
        ...
        <el-table-column label="供应商匹配" width="140">
          <template #default="{ row }">
            <el-tag :type="getSupplierMatchTagType(row.supplierMatchStatus)">
              {{ formatSupplierMatchStatus(row.supplierMatchStatus) }}
            </el-tag>
          </template>
        </el-table-column>
        ...
        <el-table-column label="正式入库单号" min-width="180">
          <template #default="{ row }">
            <el-button
              v-if="row.confirmedOrderId"
              link
              type="success"
              @click="handleViewInboundOrder(row.confirmedOrderId)"
            >
      ...
      <div v-loading="detailLoading">
        <template v-if="detailData">
          <el-descriptions :column="2" border style="margin-bottom: 16px">
            ...
            <el-descriptions-item label="匹配供应商ID">
              {{ detailData.matchedSupplierId || '-' }}
            </el-descriptions-item>
            <el-descriptions-item label="供应商匹配">
              <el-tag :type="getSupplierMatchTagType(detailData.supplierMatchStatus)">
                {{ formatSupplierMatchStatus(detailData.supplierMatchStatus) }}
              </el-tag>
            </el-descriptions-item>
            ...
            <el-descriptions-item label="关联入库单ID">
              <template v-if="detailData.confirmedOrderId">
                <el-button
                  link
                  type="primary"
                  @click="handleViewInboundOrder(detailData.confirmedOrderId)"
                >
            ...
            <el-descriptions-item label="正式入库单号">
              <template v-if="detailData.confirmedOrderId">
                <el-button
                  link
                  type="primary"
...
import { getAiInboundRecordList, getAiInboundRecordDetail } from '../../api/ai'
...
const formatStatus = (status) => {
  if (status === 'confirmed') return '已确认生成入库单'
...
const formatSupplierMatchStatus = (status) => {
...
const getSupplierMatchTagType = (status) => {
...
const openRecordDetail = async (id) => {
  ...
  try {
    const res = await getAiInboundRecordDetail(id)
    if (res.data?.code === 1) {
...
const handleViewInboundOrder = (orderId) => {
  if (!orderId) return
  detailDialogVisible.value = false
  router.push(`/inbound/detail/${orderId}`)
}
...
const handleDetailDialogClosed = async () => {
  detailData.value = null
  if (route.path !== '/ai/inbound/list' || !route.query.recordId) {
    return
  }
  await router.replace({ path: '/ai/inbound/list' })
}
